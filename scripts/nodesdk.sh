...
NUM_FILES_CHANGED=$(git diff --name-only $GIT_COMMIT | egrep 'sdk/node' | wc -l)
NUM_PROTO_FILES_CHANGED=$(git diff --name-only $GIT_COMMIT | egrep 'proto$' | wc -l)
git diff --name-only $GIT_COMMIT $GIT_BRANCH | egrep 'sdk/node'
git diff --name-only $GIT_COMMIT $GIT_BRANCH | egrep 'proto$'
